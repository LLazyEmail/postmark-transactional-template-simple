// src/templates/password-reset.definition.ts
import type {
...
/**
 * Stable registry ID. Kept as a named export so both the legacy
 * `require('./password-reset.definition')` callers and the new
 * unified registry resolve the same key.
 */
 ...
 * PasswordResetEmail
 * ------------------
 * Typed conversion of the legacy `password-reset.definition.js`.
 *
 * Public shape is `ITemplate<PasswordResetEmailProps>` — `{ name, render }`.
 * The registry normalizes this to `{ id, name, render }`.
 *
 * `name` is used for human-friendly lookup; `TEMPLATE_ID` is the
 * legacy-compatible kebab-case alias. Both resolve to this template.
 */
export const passwordReset: PasswordResetEmailTemplate = {
                              ...
                              <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                                <tr>
                                  <td class="attributes_item">
                                    <span class="f-fallback"><strong>Operating System:</strong> ${operating_system}</span>
                                  </td>
                                </tr>
                                <tr>
                                  <td class="attributes_item">
                                    <span class="f-fallback"><strong>Browser:</strong> ${browser_name}</span>
                                  </td>
                                </tr>
                              </table>
                            </td>
